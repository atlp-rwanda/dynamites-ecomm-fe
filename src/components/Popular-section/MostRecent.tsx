...
const MostRecent: React.FC = () => {
  ...
  const { items, status } = useSelector((state: RootState) => state.Popularproducts);
 ...
 const handleRightallowclick = async()=>{
  ...
  const mostRecentProducts = [...items].sort((a, b) => new Date(b.updatedAt).getTime() - new Date(a.createdAt).getTime()).slice(start, end)
  ...
  return (
    <div className=' flex flex-col mb-6'>
      <div className=' flex justify-between'>
        <div className=''>
            <p className=' font-semibold text-xl'>Recent Products</p>
            <p className='mb-4 border-b-primary border-b-4 pb-4'></p>
        </div>
        <div className=' flex h-fit w-fit mt-2'>
            <img src={leftIcon} alt="Left Allow Icon" onClick={handleLeftallowclick}/>
            <img src={righttIcon} alt="Right Allow Icon" onClick={handleRightallowclick} />
        </div>
