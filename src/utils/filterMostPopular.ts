...
export const filterPopular = (CopyOfreview: Review[], items: Product[]) => {
  const reviewReting: {
    [productName: string]: { totalRating: number; count: number };
  } = {};
  ...
  const sortedProductNames = Object.keys(reviewReting)
    .map((productName) => ({
      name: productName,
      averageRating:
        reviewReting[productName].totalRating / reviewReting[productName].count,
    }))
    .sort((a, b) => b.averageRating - a.averageRating)
    .map((product) => product.name);

  const mostPopularProducts = sortedProductNames
    .map((productName) => {
